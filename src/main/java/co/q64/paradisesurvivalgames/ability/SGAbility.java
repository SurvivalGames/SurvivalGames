...
public class SGAbility {
    ...
    public boolean hasAbility(Player p) {
        try {
            if (SGApi.getArenaManager().getArena(p).getState().equals(SGArena.ArenaState.IN_GAME) || SGApi
                    .getArenaManager().getArena(p).getState().equals(SGArena.ArenaState.DEATHMATCH)) {
    ...
    public boolean hasAbility(String p) {
        try {
            if (SGApi.getArenaManager().getArena(Bukkit.getPlayer(p)).getState().equals(SGArena.ArenaState.IN_GAME)
                    || SGApi.getArenaManager().getArena(Bukkit.getPlayer(p)).getState().equals(SGArena.ArenaState
                    .DEATHMATCH)) {
