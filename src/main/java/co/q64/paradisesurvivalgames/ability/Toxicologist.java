...
public class Toxicologist extends SGAbility implements Listener {
    ...
    public void onInteract(final PlayerInteractEvent event) {
        ...
        if (this.hasAbility(player)) {
            ...
            if (player.getItemInHand().getType() == Material.CLAY_BALL && player.getItemInHand().getItemMeta()
                    .getDisplayName().equalsIgnoreCase("Toxin Bomb")) {
                this.removeOneFromHand(player);
                FireworkUtil.getCircleUtil().playFireworkCircle(event.getPlayer(), FireworkEffect.builder().withColor
                        (Color.SILVER).withFade(Color.GREEN).flicker(true).trail(false).build(), 10, 10);
