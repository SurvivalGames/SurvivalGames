...
public class Zelda extends SGAbility implements Listener {
    ...
    public void onInteract(PlayerInteractEvent event) {
        ...
        if (this.hasAbility(player)) {
            if (player.getItemInHand().getType() == Material.SPECKLED_MELON && player.getItemInHand().getItemMeta()
                    .getDisplayName().equalsIgnoreCase("Zelda Heart")) {
                ...
                FireworkEffect fEffect = FireworkEffect.builder().flicker(false).withColor(Color.YELLOW).withFade
                        (Color.GREEN).with(Type.BALL).trail(true).build();
                try {
                    FireworkEffectPlayer.getFireworkEffectPlayer().playFirework(event.getPlayer().getWorld(),
                            event.getPlayer().getLocation(), fEffect);
                } catch (Exception e) {
