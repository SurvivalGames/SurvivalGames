            ...
            if (partyID != null) {
                ...
                if (party != null) {
                    if (SGApi.getPartyManager().getParties().get(partyID).hasRoom()) {
                        ...
                        sender.sendMessage(org.bukkit.ChatColor.YELLOW + I18N.getLocaleString("YOU_JOINED") + party
                                .getLeader() + I18N.getLocaleString("PARTY"));
                        Player player = Bukkit.getServer().getPlayer(party.getLeader());
                        player.sendMessage(org.bukkit.ChatColor.YELLOW + sender.getName() + I18N.getLocaleString
                                ("HAS_JOINED_YOUR_PARTY"));
                        for (String member : party.getMembers()) {
                            if (member != null) {
                                Player p = Bukkit.getServer().getPlayer(member);
                                if (p != null) {
                                    p.sendMessage(org.bukkit.ChatColor.YELLOW + sender.getName() + I18N
                                            .getLocaleString("HAS_JOINED_YOUR_PARTY"));
                                }
                            }
                        }
                    } else {
