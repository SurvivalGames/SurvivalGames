...
import co.q64.paradisesurvivalgames.event.PlayerKilledEvent;
...
import co.q64.paradisesurvivalgames.util.DeathMessages;
...
public class EntityDamageListener implements Listener {
    ...
    public void onEntityDamageByEntity(final EntityDamageByEntityEvent event) {
        ...
        if (SGApi.getPlugin().getPluginConfig().doBloodEffect()) {
            Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(TheSurvivalGames.getPlugin(TheSurvivalGames
                    .class), new Runnable() {
                @Override
                public void run() {
                    for (int i = 0; i < event.getDamage(); i++) {
                        event.getDamager().getWorld().playEffect(event.getEntity().getLocation().add(0.0D, 0.8D,
                                0.0D), Effect.STEP_SOUND, Material.REDSTONE_WIRE);
                    }
                }
            });
        }
        ...
        if (entity instanceof Player) {
            ...
            if (damager.getItemInHand().containsEnchantment(new ShockingEnchantment(120))) {
                FireworkEffect fEffect = FireworkEffect.builder().flicker(false).withColor(Color.BLACK).withFade
                        (Color.RED).with(Type.BALL).trail(true).build();
                try {
                    FireworkEffectPlayer.getFireworkEffectPlayer().playFirework(event.getEntity().getWorld(),
                            event.getEntity().getLocation(), fEffect);
                } catch (Exception e) {
        ...
        if (event.getEntity() instanceof Player) {
            ...
            if (eentity instanceof Player) {
                ...
                try {
                    ...
                    if (SGApi.getTimeManager(SGApi.getArenaManager().getArena(damaged)).getGameTime().count > 30) {
                        if (!SendWebsocketData.music.containsKey(damaged.getName()) || SendWebsocketData.music.get
                                (damaged.getName()).equalsIgnoreCase("ambient")) {
                            SendWebsocketData.music.put(damaged.getName(), "battle");
                            SendWebsocketData.playMusicToPlayer(damaged, SendWebsocketData.getRandomMusic
                                    ("battle-music"));
                        }
                        if (!SendWebsocketData.music.containsKey(damaged.getName()) || SendWebsocketData.music.get
                                (damaged.getName()).equalsIgnoreCase("ambient")) {
            ...
            if ((damaged.getHealth() - event.getDamage()) <= 0) {
                ...
                TheSurvivalGames.getPlugin(TheSurvivalGames.class).getServer().broadcastMessage(ChatColor
                        .translateAlternateColorCodes('&', "&e&l" + damaged.getDisplayName() + " &r&6" + I18N
                                .getLocaleString("FAIL") + " &e&l" + event.getDamager()));
            }
        }
        return;
    }
    ...
    public void onEntityDamage(EntityDamageEvent event) {
        ...
        if (event.getEntity() instanceof Player) {
            if (SGApi.getArenaManager().isInGame((Player) event.getEntity())) {
                try {
                    if (SGApi.getArenaManager().getArena((Player) event.getEntity()).getState() == SGArena.ArenaState
                            .WAITING_FOR_PLAYERS || SGApi.getArenaManager().getArena((Player) event.getEntity())
                            .getState() == SGArena.ArenaState.STARTING_COUNTDOWN) {
                        event.setCancelled(true);
                        return;
                    }
        ...
        if (entity instanceof Player) {
            ...
            if ((damaged.getHealth() - event.getDamage()) <= 0) {
                if (SGApi.getArenaManager().isInGame(damaged)) {
                    ...
                    TheSurvivalGames.getPlugin(TheSurvivalGames.class).getServer().broadcastMessage(ChatColor
                            .translateAlternateColorCodes('&', "&e&l" + damaged.getDisplayName() + " &r&6" + I18N
                                    .getLocaleString("FAIL") + " &e&l" + event.getCause().toString()));
                }
                event.setCancelled(true);
            ...
            if (entity instanceof Player) {
                ...
                try {
                    ...
                    if (damager.getInventory().getItemInHand() != null) {
                        String name = damager.getInventory().getItemInHand().getItemMeta().hasDisplayName() ? damager
                                .getInventory().getItemInHand().getItemMeta().getDisplayName() : damager.getInventory
                                ().getItemInHand().getType().toString().replace("_", " ").toLowerCase();
                        SGApi.getArenaManager().getArena(damager).broadcast(ChatColor.translateAlternateColorCodes
                                ('&', "&e&l" + damaged.getDisplayName() + " &r&6" + I18N.getLocaleString("KILLED_BY")
                                        + " &e&l" + damager.getDisplayName() + " &r&6" + I18N.getLocaleString
                                        ("WITH_A") + " &e&l" + name));
                    } else {
                        SGApi.getArenaManager().getArena(damager).broadcast(ChatColor.translateAlternateColorCodes
                                ('&', "&e&l" + damaged.getDisplayName() + " &r&6" + I18N.getLocaleString("KILLED_BY")
                                        + " &e&l" + damager.getDisplayName() + " &r&6" + I18N.getLocaleString("WITH_A") + " &e&l" + "fist"));
                    }
