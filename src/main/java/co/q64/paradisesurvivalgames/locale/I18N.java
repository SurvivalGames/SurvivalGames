...
public class I18N {
    ...
    private static void getLocaleProperties(String file) {
        ...
        if (file.equalsIgnoreCase("enUS")) {
            try {
                locales.load(new InputStreamReader(new FileInputStream(dir.getAbsolutePath() + File.separator + "enUS" +
                        ".lang"), "UTF8"));
            } catch (IOException e) {
            ...
            try {
                locales.load(new InputStreamReader(new FileInputStream(dir.getAbsolutePath() + File.separator + file
                        + ".lang"), "UTF8"));
            } catch (IOException e) {
                Bukkit.getLogger().log(Level.SEVERE, "[i18n] Could not load language file", e);
            }
    ...
    private static void addFiles() {
            ...
            if (file.endsWith(".lang")) {
                try {
                    if (!file.equalsIgnoreCase("enUS.lang")) {
                        tmp.clear();
                        tmp.load(new InputStreamReader(new FileInputStream(dir.getAbsolutePath() + File.separator +
                                file), "UTF8"));
        ...
        try {
            fallback.clear();
            tmp.load(new InputStreamReader(new FileInputStream(dir.getAbsolutePath() + File.separator + "enUS.lang"),
                    "UTF8"));
