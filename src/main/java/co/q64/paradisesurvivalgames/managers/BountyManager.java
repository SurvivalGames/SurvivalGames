    ...
    public void addBounty(int bounty, Player reciever, Player sender) {
        if (arena.getPlayers().contains(reciever.getName())) {
            try {
                ...
                reciever.sendMessage(SGApi.getArenaManager().getError() + I18N.getLocaleString
                        ("BOUNTY_SUCCESS_RECEIVED").replace("%amount", amount.toString()).replace("%sender",
                        sender.getName()));
                sender.sendMessage(SGApi.getArenaManager().getError() + I18N.getLocaleString("BOUNTY_SUCCESS_SENT")
                        .replace("%amount", amount.toString()).replace("%receiver", reciever.getName()));
            } catch (Exception e) {
