            ...
            menus.add(new IconMenu("Select Your Kit - " + i, 54, false, new IconMenu.OptionClickEventHandler() {
                ...
                public void onOptionClick(IconMenu.OptionClickEvent event) {
                    if (event.getName().startsWith("Page")) {
                        displayKitSelectionMenu(event.getPlayer(), (Integer.parseInt(event.getName().charAt(5) + "")
                                - 1));
                        return;
                    }
                    if (event.getPlayer().hasPermission("sg.kits.*") || event.getPlayer().hasPermission("sg.kits." +
                            event.getName()) || event.getPlayer().isOp()) {
                    ...
                    event.setWillClose(true);
                    event.getPlayer().sendMessage(ChatColor.RED + "Sorry, but you do not have permission to use this " +
                            "kit!");
    ...
    public void readKitsFromFiles() {
        ...
        for (String file : files) {
            if (file.startsWith("kit_")) {
                FileConfiguration kitData = YamlConfiguration.loadConfiguration(new File(SGApi.getPlugin()
                        .getDataFolder(), "kits/" + file));
                ...
                Bukkit.getServer().getLogger().info("Attempting to read inventory string of " + kitName + ". If it " +
                        "errors here, its a problem with this kit.");
