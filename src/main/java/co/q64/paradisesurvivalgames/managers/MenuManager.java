        ...
        joinMenu = new IconMenu("Join an arena", 54, true, new IconMenu.OptionClickEventHandler() {
            ...
            public void onOptionClick(OptionClickEvent event) {
                if (event.getItem().getType() == Material.EMERALD_BLOCK) {
                    try {
                        SGApi.getArenaManager().addPlayer(event.getPlayer(), Integer.parseInt(event.getName().charAt
                                (11) + ""));
        ...
        Bukkit.getScheduler().scheduleSyncRepeatingTask(SGApi.getPlugin(), new Runnable() {
            ...
            public void run() {
                ...
                for (SGArena a : arenas) {
                    if (index == 5)
                        break;
                    if (a.getState().equals(SGArena.ArenaState.WAITING_FOR_PLAYERS) || a.getState().equals(SGArena
                            .ArenaState.PRE_COUNTDOWN)) {
                        joinMenu.setOption(11 + index, new ItemStack(Material.EMERALD_BLOCK, a.getPlayers().size()),
                                "SG - Arena " + a.getId(), new String[] { ChatColor.BLACK + "",
                                        ChatColor.YELLOW + "Players: " + ChatColor.WHITE + a.getPlayers().size(),
                                        ChatColor.YELLOW + "Status: " + ChatColor.GREEN + a.getState().toString(),
                                        ChatColor.AQUA + "", ChatColor.WHITE + "" + ChatColor.UNDERLINE + "Click to " +
                                        "Join!" });
                        index++;
                    }
                ...
                for (SGArena a : arenas) {
                    if (index == 27)
                        break;
                    if (a.getState().equals(SGArena.ArenaState.IN_GAME) || a.getState().equals(SGArena.ArenaState
                            .DEATHMATCH)) {
                        joinMenu.setOption(26 + index, new ItemStack(Material.GOLD_BLOCK, a.getPlayers().size()),
                                "SG - Arena " + a.getId(), new String[] { ChatColor.BLACK + "",
                                        ChatColor.YELLOW + "Players: " + ChatColor.WHITE + a.getPlayers().size(),
                                        ChatColor.YELLOW + "Status: " + ChatColor.GREEN + a.getState().toString(),
                                        ChatColor.AQUA + "", ChatColor.WHITE + "" + ChatColor.UNDERLINE + "Click to " +
        ...
        voteMenu = new IconMenu("Vote for a map", 9, false, new IconMenu.OptionClickEventHandler() {
            ...
            public void onOptionClick(OptionClickEvent event) {
                if (event.getItem().getType() == Material.EMPTY_MAP) {
                    try {
                        try {
                            SGApi.getArenaManager().getArena(event.getPlayer()).vote(event.getPlayer(),
                                    Integer.parseInt(event.getItem().getItemMeta().getLore().get(0).charAt(4) + ""));
    ...
    public void displayVoteMenu(Player p) {
        ...
        try {
            ...
            for (Map.Entry<MapHash, Integer> entry : arena.getVotes().entrySet()) {
                voteMenu.setOption(i, new ItemStack(Material.EMPTY_MAP), entry.getKey().getWorld().getDisplayName(),
                        new String[] { "Map " + entry.getKey().getId(), "Current Votes: " + entry.getValue() });
                i++;
            }
