        ...
        Bukkit.getScheduler().scheduleSyncDelayedTask(SGApi.getPlugin(), new Runnable() {
            ...
            public void run() {
                if (WebsocketSessionManager.getSessionManager().getSessionByName(p.getName()) != null) {
                    WebsocketServer.s.sendData(WebsocketSessionManager.getSessionManager().getSessionByName(p.getName
                            ()), "sound:" + data);
                }
            }
        });
        ...
        Bukkit.getScheduler().scheduleSyncDelayedTask(SGApi.getPlugin(), new Runnable() {
            ...
            public void run() {
                for (String s : arena.getPlayers()) {
                    ...
                    if (WebsocketSessionManager.getSessionManager().getSessionByName(p.getName()) != null) {
                        WebsocketServer.s.sendData(WebsocketSessionManager.getSessionManager().getSessionByName(p
                                .getName()), "sound:" + data);
                    }
                }
            }
    ...
    public static void updateArenaStatusForPlayer(final Player p) {
        Bukkit.getScheduler().scheduleSyncDelayedTask(SGApi.getPlugin(), new Runnable() {
            ...
            public void run() {
                ...
                WebsocketServer.s.sendData(WebsocketSessionManager.getSessionManager().getSessionByName(p.getName()),
                        "points:" + EconUtil.getPoints(p));
                WebsocketServer.s.sendData(WebsocketSessionManager.getSessionManager().getSessionByName(p.getName()),
                        "kills:" + data.getKills());
                WebsocketServer.s.sendData(WebsocketSessionManager.getSessionManager().getSessionByName(p.getName()),
                        "wins:" + data.getWins());
                WebsocketServer.s.sendData(WebsocketSessionManager.getSessionManager().getSessionByName(p.getName()),
                        "rank:" + ChatColor.stripColor(ChatColor.translateAlternateColorCodes('&', data.getRank())));
                SGArena a = null;
                try {
                ...
                WebsocketServer.s.sendData(WebsocketSessionManager.getSessionManager().getSessionByName(p.getName()),
                        "alive:" + join(a.getPlayers(), "<br>"));
                WebsocketServer.s.sendData(WebsocketSessionManager.getSessionManager().getSessionByName(p.getName()),
                        "dead:" + join(a.getSpectators(), "<br>"));
                WebsocketServer.s.sendData(WebsocketSessionManager.getSessionManager().getSessionByName(p.getName()),
                        "specs:" + join(a.getSpectators(), "<br>"));
            }
        });
