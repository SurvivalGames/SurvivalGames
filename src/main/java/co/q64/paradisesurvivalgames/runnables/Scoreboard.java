...
public class Scoreboard implements Runnable {
    ...
    private void createScoreboard(Player player) {
        ...
        objective.setDisplayName(ChatColor.translateAlternateColorCodes('&', "&a&l" + I18N.getLocaleString("WELCOME")
                + ", " + player.getName()));
        ...
        if (player.isOnline()) {
            try {
                ...
                Bukkit.getLogger().log(Level.SEVERE, I18N.getLocaleString("COULD_NOT_CREATE") + " " + player
                        .getDisplayName());
                return;
            }
    ...
    private void updateScoreboard(Player player, boolean complete) {
        ...
        if (!SGApi.getArenaManager().isInGame(player)) {
            ...
            objective.setDisplayName(ChatColor.translateAlternateColorCodes('&',
                    "&a&l" + I18N.getLocaleString("WELCOME") + ", " + player.getName()));
        ...
        if (arena.getState() == SGArena.ArenaState.WAITING_FOR_PLAYERS) {
            objective.setDisplayName(ChatColor.translateAlternateColorCodes('&',
                    "&a&l" + I18N.getLocaleString("WAITING_FOR_PLAYERS")));
        ...
        if (arena.getState() == SGArena.ArenaState.PRE_COUNTDOWN) {
            objective.setDisplayName(ChatColor.translateAlternateColorCodes('&',
                    "&a&l" + I18N.getLocaleString("Starting in: " + SGApi.getTimeManager(arena).getG().timeToString()) + "" +
                            " " + SGApi.getTimeManager(arena).getG().s[1]));
        ...
        objective.setDisplayName(ChatColor.translateAlternateColorCodes('&',
                "&a&l" + I18N.getLocaleString("SURVIVAL_GAMES")));
